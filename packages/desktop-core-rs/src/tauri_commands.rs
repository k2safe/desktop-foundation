...
#[tauri::command]
pub async fn df_http_request(
    core: State<'_, DesktopCore>,
    request: HttpRequest,
) -> Result<HttpResponse, DesktopError> {
...
#[tauri::command]
pub async fn df_session_get(
    core: State<'_, DesktopCore>,
    request: SessionGetRequest,
) -> Result<SessionState, DesktopError> {
    core.session_get(request)
}

#[tauri::command]
pub async fn df_session_set(
    core: State<'_, DesktopCore>,
...
#[tauri::command]
pub async fn df_storage_get(
    core: State<'_, DesktopCore>,
    request: StorageGetRequest,
) -> Result<StorageValue, DesktopError> {
    core.storage_get(request)
}

#[tauri::command]
pub async fn df_storage_set(
    core: State<'_, DesktopCore>,
...
#[tauri::command]
pub async fn df_storage_remove(
    core: State<'_, DesktopCore>,
...
#[tauri::command]
pub async fn df_file_open_dialog(
    core: State<'_, DesktopCore>,
    request: OpenFileDialogRequest,
) -> Result<FileDialogReply, DesktopError> {
    core.open_file_dialog(request)
}

#[tauri::command]
pub async fn df_file_save_dialog(
    core: State<'_, DesktopCore>,
    request: SaveFileDialogRequest,
) -> Result<SaveFileDialogReply, DesktopError> {
    core.save_file_dialog(request)
}

#[tauri::command]
pub async fn df_file_read_text(
    core: State<'_, DesktopCore>,
    request: ReadTextFileRequest,
) -> Result<TextFileReply, DesktopError> {
    core.read_text_file(request)
}

#[tauri::command]
pub async fn df_file_write_text(
    core: State<'_, DesktopCore>,
    request: WriteTextFileRequest,
) -> Result<FilePathReply, DesktopError> {
    core.write_text_file(request)
}

#[tauri::command]
pub async fn df_file_export_json(
    core: State<'_, DesktopCore>,
...
#[tauri::command]
pub async fn df_file_download(
    core: State<'_, DesktopCore>,
    request: DownloadFileRequest,
) -> Result<DownloadFileReply, DesktopError> {
    let core = core.inner().clone();
    tauri::async_runtime::spawn_blocking(move || core.download_file(request))
        .await
        .map_err(|error| {
            DesktopError::new("DOWNLOAD_TASK_FAILED", "Download task failed")
                .with_details(serde_json::Value::String(error.to_string()))
        })?
}

#[tauri::command]
pub async fn df_update_install(
    core: State<'_, DesktopCore>,
    request: UpdateInstallRequest,
) -> Result<UpdateInstallReply, DesktopError> {
    let core = core.inner().clone();
    tauri::async_runtime::spawn_blocking(move || core.install_update(request))
        .await
        .map_err(|error| {
            DesktopError::new("UPDATE_INSTALL_TASK_FAILED", "Update install task failed")
                .with_details(serde_json::Value::String(error.to_string()))
        })?
}

#[tauri::command]
pub async fn df_secure_storage_get(
    core: State<'_, DesktopCore>,
...
#[tauri::command]
pub async fn df_secure_storage_set(
    core: State<'_, DesktopCore>,
...
#[tauri::command]
pub async fn df_secure_storage_remove(
    core: State<'_, DesktopCore>,
...
pub async fn df_window_get_state(window: tauri::Window) -> Result<WindowState, DesktopError> {
    let position = window.outer_position().map_err(|error| {
        DesktopError::new("WINDOW_POSITION_FAILED", "Failed to read window position")
            .with_details(serde_json::Value::String(error.to_string()))
    })?;
    let size = window.outer_size().map_err(|error| {
        DesktopError::new("WINDOW_SIZE_FAILED", "Failed to read window size")
            .with_details(serde_json::Value::String(error.to_string()))
    })?;
    let maximized = window.is_maximized().map_err(|error| {
        DesktopError::new("WINDOW_MAXIMIZED_FAILED", "Failed to read maximized state")
            .with_details(serde_json::Value::String(error.to_string()))
    })?;
    let fullscreen = window.is_fullscreen().map_err(|error| {
        DesktopError::new(
            "WINDOW_FULLSCREEN_FAILED",
...
#[tauri::command]
pub async fn df_window_set_state(
    window: tauri::Window,
    request: WindowSetStateRequest,
) -> Result<DesktopActionReply, DesktopError> {
    if let (Some(width), Some(height)) = (request.width, request.height) {
        window
            .set_size(tauri::Size::Physical(tauri::PhysicalSize { width, height }))
            .map_err(|error| {
                DesktopError::new("WINDOW_SET_SIZE_FAILED", "Failed to set window size")
                    .with_details(serde_json::Value::String(error.to_string()))
            })?;
    }
    ...
    if let Some(maximized) = request.maximized {
        if maximized {
            window.maximize().map_err(|error| {
                DesktopError::new("WINDOW_MAXIMIZE_FAILED", "Failed to maximize window")
                    .with_details(serde_json::Value::String(error.to_string()))
            })?;
        } else {
            window.unmaximize().map_err(|error| {
                DesktopError::new("WINDOW_UNMAXIMIZE_FAILED", "Failed to unmaximize window")
                    .with_details(serde_json::Value::String(error.to_string()))
            })?;
        }
    }
    if let Some(fullscreen) = request.fullscreen {
        window.set_fullscreen(fullscreen).map_err(|error| {
            DesktopError::new("WINDOW_FULLSCREEN_FAILED", "Failed to set fullscreen state")
                .with_details(serde_json::Value::String(error.to_string()))
        })?;
    }
...
#[tauri::command]
pub async fn df_window_set_title(
    window: tauri::Window,
    request: WindowSetTitleRequest,
) -> Result<DesktopActionReply, DesktopError> {
    window.set_title(&request.title).map_err(|error| {
        DesktopError::new("WINDOW_SET_TITLE_FAILED", "Failed to set window title")
            .with_details(serde_json::Value::String(error.to_string()))
    })?;
    Ok(DesktopActionReply { ok: true })
}
